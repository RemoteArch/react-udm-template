...
export default function Home() {
  ...
  const saveToHistory = (dbName, token) => {
    ...
    newHistory.unshift({ dbName, token, date: new Date().toLocaleDateString() });
  ...
  // Connexion avec token manuel
  const handleTokenLogin = () => {
    if (!tokenInput) return;
    
    try {
      const decoded = decodeURIComponent(escape(atob(tokenInput)));
      const dbName = decoded.split('name||')[1].split('|||')[0];
      saveToHistory(dbName, tokenInput);
    ...
    <div className="bg-black min-h-screen text-gray-200 font-sans p-4 md:p-8">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="glass-panel p-8 rounded-2xl shadow-2xl" style={{
          ...
          <div className="glass-panel p-6 rounded-2xl shadow-xl border-t-4 border-blue-500" style={{
            background: 'rgba(31, 41, 55, 0.7)',
            backdropFilter: 'blur(12px)',
            border: '1px solid rgba(255, 255, 255, 0.1)'
          }}>
        ...
        <div className="lg:col-span-5">
          <div className="glass-panel h-full flex flex-col rounded-2xl shadow-2xl overflow-hidden" style={{
            background: 'rgba(31, 41, 55, 0.7)',
            backdropFilter: 'blur(12px)',
            border: '1px solid rgba(255, 255, 255, 0.1)'
          }}>
            <div className="p-6 border-b border-gray-700 flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-400">{history.length} config{history.length > 1 ? 's' : ''}</span>
              </div>
            </div>
            <div 
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="font-bold text-white text-sm uppercase tracking-tighter truncate">{item.dbName}</p>
                          <p className="text-[10px] text-gray-500 font-mono truncate">{item.token.substring(0, 20)}...</p>
                          <p className="text-[9px] text-gray-600 mt-1">Ajoutée le {item.date}</p>
                        </div>
                      </div>
