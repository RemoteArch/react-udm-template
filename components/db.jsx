...
// ─── API LAYER ────────────────────────────────────────────────────────────────
const API_URL   = window.__DB_MANAGER_APIURL__ ?? "";
const getToken  = () => window.__DB_MANAGER_TOKEN__ ?? "";
...
const API = {
  // Tables
  list:     ()             => api("GET",    "list").then(res => res.data || []),
  describe: (table)        => api("GET",    "describe", table ? { table } : {}).then(res => {
    // Si table spécifiée, retourne {create_sql, describe}
    // Sinon retourne un array de tables avec leur structure
    if (table) return res;
    return res.data || res;
  }),
  create:   (table)        => api("GET",    "create",   { table }).then(res => res),
  // Rows
  rows:     (table, p={}) => api("GET",    "rows",     { table, ...p }).then(res => ({
    rows: res.rows || [],
    total: res.total || 0
  })),
  insertRow:(table, row)  => api("POST",   "row",      { table }, row),
  updateRow:(table, row)  => api("PUT",    "row",      { table }, row),
  deleteRow:(table, pk)   => api("DELETE", "row",      { table, ...pk }),
  // DDL
  postCreate:  (data)     => api("POST",   "create",   {}, data),
  dropTable:   (table)    => api("DELETE", "drop",     { table, confirm: "true" }),
  dropAll:     ()         => api("DELETE", "all",      { confirm: "true" }),
  truncate:    (table)    => api("DELETE", "truncate", { table }, { table }),
  initDb:      (path)     => api("GET",    "init",     { path }),
  // Structure modification
  addColumn:    (data)    => api("POST",   "add_column",    {}, data),
  dropColumn:   (table, column) => api("DELETE", "drop_column", { table, column, confirm: "true" }),
  modifyColumn: (data)    => api("PUT",    "modify_column", {}, data),
  // Foreign keys
  getForeignKeys:   (table) => api("GET",    "foreign_keys", { table }).then(res => res.foreign_keys || []),
  addForeignKey:    (data)  => api("POST",   "add_foreign_key", {}, data),
  dropForeignKey:   (table, constraint) => api("DELETE", "drop_foreign_key", { table, constraint, confirm: "true" }),
  // SQL execution
  execute:          (sql)   => api("POST",   "execute", {}, { sql }),
};
...
function InitDbModal({ onClose, onDone, toast }) {
  ...
  return (
    <Modal title="Exécuter du SQL" onClose={onClose} wide>
      <div className="space-y-4">
        <div className="bg-sky-950/40 border border-sky-800/40 rounded-lg px-4 py-3 text-xs text-sky-300">
          <i className="fas fa-info-circle mr-1"></i>
          Exécute du SQL brut en transaction. Toutes les instructions seront annulées en cas d'erreur.
        </div>
        <div>
...
function TableStructureModal({ table, describe, tables, onClose, onDone, toast }) {
  ...
  async function handleAddColumn() {
    ...
    setLoading(true);
    try {
      await window.API.addColumn({
        table,
        name: columnForm.name,
        type: columnForm.type,
        nullable: columnForm.nullable ?? true,
        default: columnForm.default || null,
        after: columnForm.after || null,
      });
  ...
  async function handleModifyColumn() {
    ...
    setLoading(true);
    try {
      await window.API.modifyColumn({
        table,
        old_name: columnForm.old_name,
        new_name: columnForm.new_name,
        type: columnForm.type,
        nullable: columnForm.nullable ?? true,
        default: columnForm.default || null,
      });
  ...
  async function handleDropColumn(columnName) {
    ...
    setLoading(true);
    try {
      await window.API.dropColumn(table, columnName);
  ...
  async function handleAddForeignKey() {
    ...
    setLoading(true);
    try {
      await window.API.addForeignKey({
        table,
  ...
  async function handleDropFK(constraint) {
    ...
    setLoading(true);
    try {
      await window.API.dropForeignKey(table, constraint);
  ...
  async function loadRefTableColumns(refTable) {
    try {
      const desc = await window.API.describe(refTable);
      setRefColumns(desc.describe || []);
    } catch (e) {
      toast.error(e.message);
    }
  }
                  ...
                  {foreignKeys.map((fk, i) => (
                    <div key={i} className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="font-mono text-xs text-zinc-400 mb-1">{fk.name}</div>
                          <div className="text-sm text-zinc-300">
                            <code className="bg-zinc-900 px-1.5 py-0.5 rounded">{fk.column}</code>
                            <i className="fas fa-arrow-right mx-2 text-zinc-600"></i>
                            <code className="bg-zinc-900 px-1.5 py-0.5 rounded">{fk.ref_table}.{fk.ref_column}</code>
                          </div>
...
// ─── SIDEBAR ─────────────────────────────────────────────────────────────────
function Sidebar({ tables, active, onSelect, onRefresh, onCreateTable, onDropTable, onDropAll, onTruncate, onInitDb }) {
  return (
    <aside className="w-56 flex-shrink-0 flex flex-col bg-zinc-900 border-r border-zinc-800 h-full">
      ...
      <div className="px-3 py-3 space-y-1 border-b border-zinc-800">
        ...
        <Btn size="sm" className="w-full justify-center" onClick={onInitDb}>
          <i className="fas fa-cog"></i> Init SQL
        </Btn>
        ...
        {schema.map(s => (
          <div key={s.table} onClick={() => onSelect(s.table)}
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-zinc-600">{s.describe?.length ?? 0} colonnes</span>
            </div>
            <div className="space-y-1">
              {(s.describe ?? []).slice(0, 5).map(col => (
              ...
              {(s.describe?.length ?? 0) > 5 && (
                <p className="text-[10px] text-zinc-600 pt-1">+{s.describe.length - 5} colonnes…</p>
              )}
            </div>
          </div>
        ))}
